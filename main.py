...
def main():
    url_code = 'https://www.mashina.kg/search/all/'
    html = get_html(url_code)
    get_data(html)
    number = get_total(html)
    for i in range(1, number + 1):
        url_with_page = url_code + '&page=' + str(i)
...
with open('data.csv', 'w') as file:
   ...
   write_.writerow(['title' , 'pricebucks' , 'price' , 'image' , 'description'])
